...
struct InputDiffuser {
  ...
  inline float process(float in) {
    std::ranges::for_each(_allPass,
                          [&in](auto &allPass) { in = allPass.process(in); });
    return in;
  }
...
struct TankBranch {
  ...
  dsp::AllPassFilter _modAllPass = {44100.0F, 100.0F, 0.5F};
  dsp::AllPassFilter _allPass = {44100.0F, 100.0F, 0.5F};
  std::array<dsp::FractionalDelay, 2U> _delays = {
      dsp::FractionalDelay{100.0 * 44100.0F},
      dsp::FractionalDelay{100.0 * 44100.0F}};
...
private:
  ...
  float _predelaySamples = 410.0;
  dsp::FractionalDelay _preDelay = dsp::FractionalDelay{1000.0};
