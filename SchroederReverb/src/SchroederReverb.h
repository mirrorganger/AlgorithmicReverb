...
public:
  ...
  void process(float *const *signal, size_t numberOfChannels,
    ...
    for (size_t sample_i = 0; sample_i < framesPerBuffer; ++sample_i) {
      ...
      std::ranges::for_each(_preAllPass, [&sample](auto &allPass) {
      ...
      std::ranges::for_each(
          _allPass, [&out](auto &allPass) { out = allPass.process(out); });
...
private:
  ...
  constexpr static size_t NUM_ALLPASS = 4U;

  std::array<float, NUM_COMBS> _combsDelay_ms = {29.7F, 30.1F, 32.2F, 34.6F,
                                                 37.2F, 38.1F, 39.7F, 40.5,
                                                 41.3F, 42.6F, 43.7F, 45.6F};

  std::array<float, NUM_ALLPASS> _allPassDelay_ms = {1.0F, 2.3F, 3.7F, 5.0F};
  std::array<float, NUM_PRE_ALLPASS> _preAllPassDelay_ms = {1.0f, 2.0F};
  std::array<dsp::CombFilter, NUM_COMBS> _combs =
      make_array<NUM_COMBS>(dsp::CombFilter(44100.0, 200.0, 0.707F));
  std::array<dsp::AllPassFilter, NUM_ALLPASS> _allPass =
      make_array<NUM_ALLPASS>(dsp::AllPassFilter(44100.0, 10.0, 0.707F));
  std::array<dsp::AllPassFilter, NUM_PRE_ALLPASS> _preAllPass =
      make_array<NUM_PRE_ALLPASS>(dsp::AllPassFilter(44100.0, 10.0, 0.707F));
};
